...
> `synchronous`, `asynchronous`, `blocking`, `non-blocking` function의 차이를 아는 것은 중요하다. 잘 모르더라도 직관적이고 가독성이 높은 쪽은 말하지 않아도 명확하다. 불명확하고 1차원적으로 확인 불가능한 부분을 최소로 하도록 노력하는 것도 일종의 `기술 부채`를 줄여 나가는 것과 동일하다고 생각한다.

### Useful Test Failures
테스트는 반드시 무엇이 잘못 되었고, 무엇이 입력이었으며, 실제 결과는 무엇인지 그리고 무엇을 예상하고 있었는지를 말해주는 `도움이 되는 메시지`와 함께 실패 해야 한다. 테스트를 위한 헬퍼들을 많이 작성하는 것은 유혹적이만, 이러한 헬퍼들이 유용한 에러 메시지를 생성하는지를 확실히 해야 한다. 당신의 실패하는 테스트를 디버깅 하는 사람이 당신 자신 또는 당신의 팀이 아닌 다른 사람이라고 가정하라. 일반적인 `golang`의 테스트는 아래와 같이 실패한다:
```go
if got != tt.want {
	t.Errorf("Foo(%q) = %d; want %d", tt.in, got, tt.want) // or Fatalf, if test can't test anything more past this point
}
```
순서가 `actual != expected` 인 점을 유의하라. 에러 메시지 또한 같은 순서를 사용한다. 몇몇 테스트 프레임워크에서는 이것의 반대로(`0 != x`, "expected 0, got x") 작성하도록 장려하는데, `golang`은 그렇지 않다.  
타이핑 해야 하는 양이 많아 보인다면, `table-driven test`를 작성하는 것이 나을 것이다.  
서로 다른 인풋으로 테스트 헬퍼를 사용할 때 모호함을 줄일 수 있는 또 하나의 테크닉은 각각의 호출자를 서로 다른 `TestFoo` function으로 감쌈으로써 그 테스트의 이름으로 실패하게 하는 것이다:  
```go
func TestSingleValue(t *testing.T) { testHelper(t, []int{80}) }
func TestNoValues(t *testing.T)    { testHelper(t, []int{}) }
```
어떤 경우든, 다가올 미래에 당신의 코드를 디버깅하는 누군가가 `도움이 되는` 에러 메시지와 함께 실패하게 하는 것은 당신의 의무이다.

> 실패하는 테스트를 먼저 작성하고 그 테스트를 통과하는 코드를 구현해나가는 `TDD`를, 언제나, 하고 싶다. 저게 옳다는 것도 알고 더 나은 결과를 가져다 줄 것도 알지만 왠지 프로젝트 초반에는 당장 구현 해야 할(하고 싶은..) 코드가 눈에 보이고 빨리 실체화 하고 싶은 욕심이 앞서고 만다. (~결국 Test Following Development~)  
`테스트 없이 개발 -> 빠른 개발 속도에 흐믓 -> 스펙 변경 -> 또 스펙 변경 -> 변화의 영향을 정확히 예측하지 못함 -> 여기 저기서 이슈 리포팅 -> 땜빵 -> 반복..`의 악순환을 끊어 내려면, 코드의 변경 시점에 자동화 된 프로세스로 기존 동작에 대한 신뢰성을 확인할 수 있는 것이 중요하다. 이러한 디딤돌 없이는 그 어떤 변경도 무서울 수 밖에 없다.  
테스트를 작성하는 것 만큼이나 `제대로 된` 테스트를 작성 하는 것도 중요하다. 단지 마음의 위안을 얻기 위함이거나, 생색을 내기 위한 테스트는 안하니만 못하고, 테스트 또한 코드와 마찬가지로 생명 주기를 가지고 관리 되어야 하는 존재이기에 오히려 부담이 될 수 밖에 없다. 테스트 작성 또는 활용으로 인해 고통 받고 있거나, 테스트를 씀으로 인한 이점을 느끼지 못하고 있다면 테스트를 잘못 작성/활용하고 있지 않나를 고민해볼 필요도 있을 것이다. 나는 비록 테스트 자체를 위한 또 다른 프레임워크를 사용하는 것이 싫어서 지양하고 있지만, `golang` 테스트 생태계에는 테스트를 도와주는 수 많은 검증된 프레임워크들이 있으며, 간단하게는 `Table Based Test`와 같은 방법론으로 최소한의 룰을 가지고 의미 있는 테스트를 추구할 수 있다. 테스트는 중요하다.

### Variable Names
`golang`에서의 변수 이름은 긴 것 보다는 짧은 것을 추구해야 한다. 제한된 scope을 가진 지역 변수들에게는 더욱 그러하다. `c`를 `lineCounter`보다 선호하라. `i`를 `sliceIndex`보다 선호하라.  
기본 원칙 : 이름이 사용되는 선언에서 멀어질수록 더욱 설명적인 이름이어야 한다. `메서드 리시버`의 경우 하나 혹은 두 글자로도 충분하다. Loop의 index나 reader와 같은 일반적인 변수들의 경우 `i, r`과 같은 한 글자로 가능하다. 친숙하지 않거나 전역 변수인 경우 더욱 설명적인 이름이어야 한다.

> 마지막 코멘트여서 좋아 했는데 `변수나 함수의 이름만 보고도 이게 뭔지, 뭘 하는지 알아야 한다`라는 생각을 가진 나로서는 너무 아쉬운 코멘트이다. 좀 길면 어떤가, 내가 다 타이핑 하는 것도 아니고, 이름으로 한 줄을 넘기는 것도 아니라면. 이름이 짧아진다고 성능이 좋아지는 것도 아니다. Loop에서의 `i` 같은 경우 거의 고유명사 급으로 인지도가 있지만 이름이 짧아질 수록 다른 누군가의 가독성을 나쁘게 할 확률은 높아질 수 밖에 없다. 읽기 쉬운 코드가 잘 작성한 코드이고, 짧은 코드 보다는 가독성이 좋은 코드가 낫다.
